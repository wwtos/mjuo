...
impl Node for InputsNode {
    fn accept_stream_input(&mut self, socket_type: &StreamSocketType, value: f32) {
        let index = self.inputs.iter().position(|x| x == &SocketType::Stream(socket_type.clone())).unwrap();

        self.values[index] = SocketValue::Stream(value);
    }

    fn accept_midi_input(&mut self, socket_type: &MidiSocketType, value:Vec<MidiData>) {
        let index = self.inputs.iter().position(|x| x == &SocketType::Midi(socket_type.clone())).unwrap();
    ...
    fn accept_value_input(&mut self, socket_type: &ValueSocketType, value: Primitive) {
        let index = self.inputs.iter().position(|x| x == &SocketType::Value(socket_type.clone())).unwrap();
    ...
    fn init(
        ...
        let node_rows = self.inputs.iter().map(|socket_type| {
            NodeRow::from_type_and_direction(socket_type.clone(), SocketDirection::Input)
        }).collect::<Vec<NodeRow>>();
