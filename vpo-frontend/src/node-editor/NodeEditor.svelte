<script lang="ts">
  ...
  function handleNodeMousedown(index: NodeIndex, event: MouseEvent) {
    if (event.button === 0) {
      ...
      let node = $activeGraph.getNode(draggedNode);
      draggedOffset = [
        mouseX - node.uiData.getValue().x,
        mouseY - node.uiData.getValue().y,
      ];
  ...
  function handleSocketMousedown(
    event: MouseEvent,
    socket: SocketType,
    direction: SocketDirection,
    index: NodeIndex
    ...
    if (direction === SocketDirection.Input) {
      ...
      const disconnectingFrom = $activeGraph.getNode(index);

      let connection = disconnectingFrom.connectedInputs
        .getValue()
        .find((inputSocket) => {
          return (
            socketToKey(inputSocket.toSocketType, direction) ===
            socketToKey(socket, direction)
          );
        });
      ...
      if (connection) {
        ...
        const fromNodeXY = fromNode.getSocketXYCurrent(
          connection.fromSocketType,
  ...
  function connectionToPoints(connection: any): {
    x1: number;
    y1: number;
    x2: number;
    y2: number;
    ...
    const fromNode = $activeGraph.getNode(connection.fromNode);
    const toNode = $activeGraph.getNode(connection.toNode);

    const fromXY = fromNode.getSocketXYCurrent(
      connection.fromSocketType,
      SocketDirection.Output
    );
    const toXY = toNode.getSocketXYCurrent(
      connection.toSocketType,
