<script lang="ts">
      ...
      return nodeRows
        .map((nodeRow) => {
          return nodeRow.match({
            StreamInput: ([streamInput, def]) =>
              ReducedRowType.SocketRow(
                SocketType.Stream(streamInput),
                SocketDirection.Input,
                def
              ),
            MidiInput: ([midiInput, def]) =>
              ReducedRowType.SocketRow(
                SocketType.Midi(midiInput),
                SocketDirection.Input,
                def
              ),
            ValueInput: ([valueInput, def]) =>
              ReducedRowType.SocketRow(
                SocketType.Value(valueInput),
                SocketDirection.Input,
                def
              ),
            NodeRefInput: (nodeRefInput) =>
              ReducedRowType.SocketRow(
                SocketType.NodeRef(nodeRefInput),
                SocketDirection.Input,
                undefined
              ),
            StreamOutput: ([streamOutput, def]) =>
              ReducedRowType.SocketRow(
                SocketType.Stream(streamOutput),
                SocketDirection.Output,
                def
              ),
            MidiOutput: ([midiOutput, def]) =>
              ReducedRowType.SocketRow(
                SocketType.Midi(midiOutput),
                SocketDirection.Output,
                def
              ),
            ValueOutput: ([valueOutput, def]) =>
              ReducedRowType.SocketRow(
                SocketType.Value(valueOutput),
                SocketDirection.Output,
                def
              ),
            NodeRefOutput: (nodeRefOutput) =>
              ReducedRowType.SocketRow(
                SocketType.NodeRef(nodeRefOutput),
                SocketDirection.Output,
                undefined
              ),
  ...
  function onMousedownRaw(e: MouseEvent) {
    onMousedown(
      new NodeIndex(wrapper.index.index, wrapper.index.generation),
      e
    );
  }
  ...
  function rowToKey(row: ReducedRowType): string {
    return row.variant === "SocketRow"
      ? socketToKey(row.data[0], row.data[1])
...
<div
  class="background"
  style="transform: translate({$uiData.x}px, {$uiData.y}px); width: {width}px"
  on:mousedown={onMousedownRaw}
  class:selected={$uiData.selected}
  bind:this={node}
>
  <div class="node-title">
    {$uiData.title && $uiData.title.length > 0
      ? i18n.t("nodes." + $uiData.title)
      : " "}
  </div>
